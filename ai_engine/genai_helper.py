...
class GenAIHelper:
    """Helper untuk integrasi dengan Google GenAI API"""
    ...
    def test_connection(self) -> bool:
        """Test connection ke GenAI API"""
        if not self.client:
            return False
        
        try:
            # Test dengan simple text generation
    ...
    def upload_video(self, video_path: str, progress_callback: Optional[Callable] = None) -> Optional[Any]:
        """
        Upload video file ke GenAI
        Returns uploaded file object atau None jika gagal
        """
        if not self.client:
    ...
    def generate_prompts_batch(self, uploaded_file: Any, complexity_level: int, 
        ...
        """
        Generate multiple prompts dalam 1 request
        
        Args:
            uploaded_file: File yang sudah diupload ke GenAI
            complexity_level: Level kompleksitas (1-5)
            aspect_ratio: Aspect ratio target
            variation_level: Level variasi (1-5)
            prompts_count: Jumlah prompts yang diminta
        """
        if not self.client:
            raise RuntimeError("GenAI client not initialized")
        
        # Get complexity descriptions
        ...
        # Build batch prompt berdasarkan parameters
        ...
        try:
            ...
            # Parse response untuk extract multiple prompts
    ...
    def _build_batch_prompt(self, complexity_desc: str, aspect_ratio: str, 
        ...
        """Build batch prompt untuk multiple generation"""
        
        # Get variation instructions dari config
        variation_instructions = self.config.get_variation_instructions()
        variation_instruction = variation_instructions.get(str(variation_level), 
                                                          "Generate with moderate variation")
...
Make sure each prompt is detailed, creative, and suitable for AI art generation tools."""
    ...
    def _parse_batch_response(self, response_text: str, expected_count: int) -> List[str]:
        """Parse batch response untuk extract multiple prompts"""
        import json
        import re
        ...
        # Jika JSON parsing gagal, coba extract JSON dari text
        ...
        for line in lines:
            line = line.strip()
            # Look for numbered prompts like "1.", "2.", etc.
            if re.match(r'^\d+\.', line):
                prompt = re.sub(r'^\d+\.\s*', '', line).strip()
                if prompt and prompt.startswith('"') and prompt.endswith('"'):
                    prompt = prompt[1:-1]  # Remove quotes
                prompts.append(prompt)
                if len(prompts) >= expected_count:
                    break
        
        # Jika masih tidak ada, return response as single prompt
        if not prompts:
    ...
    def _build_base_prompt(self, complexity_desc: str, aspect_ratio: str, 
        ...
        """Build base prompt untuk generation"""
        
        # Get variation instructions dari config
        variation_instructions = self.config.get_variation_instructions()
        variation_instruction = variation_instructions.get(str(variation_level), 
                                                         variation_instructions.get("3", "Balance original content with moderate creative variations."))
...
Generate only the final prompt, no additional explanation or analysis.
    ...
    def generate_multiple_prompts(self, video_path: str, num_prompts: int,
        ...
        """
        Generate multiple prompts untuk satu video dengan auto-split batch
        Otomatis memecah request besar menjadi beberapa batch kecil
        ...
        # Get max prompts per batch from config
        max_prompts_per_batch = self.config.get("generation.max_prompts_per_batch", 5)
        ...
        try:
            ...
            for batch_num in range(num_batches):
                # Calculate prompts for this batch
            ...
            # Return exactly the number requested, or all we got
            ...
            # Cleanup uploaded file jika perlu
            if uploaded_file:
                try:
    ...
    def batch_generate_prompts(self, videos: List[Dict[str, Any]], 
        ...
        """
        Batch generate prompts untuk multiple videos
        ...
        for i, video in enumerate(videos):
            try:
                ...
                results[video['id']] = []  # Empty list untuk error
    ...
    def update_api_key(self, new_api_key: str) -> bool:
        """Update API key dan reinitialize client"""
        try:
...
def get_genai_helper() -> GenAIHelper:
    """Get singleton instance dari GenAIHelper"""
