...
class SettingsDialog(QDialog):
    ...
    def create_generation_tab(self) -> QWidget:
        ...
        # Max prompts per video
        self.max_prompts_spinbox = QSpinBox()
        self.max_prompts_spinbox.setRange(min_prompts, max_prompts * 5)  # Allow higher max
    ...
    def load_current_settings(self):
        ...
        try:
            # API settings - get from .env via get_api_key()
            try:
                api_key = self.config.get_api_key()
            except ValueError:
                api_key = ""  # API key not configured yet
    ...
    def save_settings(self):
        """Save settings ke config file"""
        try:
            # API settings - save to .env using set_api_key()
    ...
    def test_api_connection(self):
        ...
        try:
            # Temporarily update API key to .env for test
            try:
            ...
            # Import and test GenAI helper
            from ai_engine.genai_helper import GenAIHelper
            
            # Create fresh instance untuk test
